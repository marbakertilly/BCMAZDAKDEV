report 50101 "SalesInvoiceBT_2"
{
    ...
    dataset
    {
        dataitem(Header; "Sales Invoice Header")
        {
            ...
            column(CustomerNo_Lbl2; CustomerNoLbl2)
            {
            }
            ...
            dataitem(Line; "Sales Invoice Line")
            {
                ...
                column(Text_Lbl; TextLbl)
                {
                }
                column(Amount_Lbl; AmountLbl)
                {
                }
                column(Subtotal1_Lbl; Subtotal1Lbl)
                {
                }
                column(Total1_Lbl; Total1Lbl)
                {
                }
                column(PaymentTerms_Lbl; PaymentTermsLbl)
                {
                }
                column(PaymentWithin_Lbl; PaymentWithinLbl)
                {
                }
                column(AfterDueDate_Lbl; AfterDueDateLbl)
                {
                }
                column(InterestTerms_Lbl; InterestTermsLbl)
                {
                }
                column(BankPayment_Lbl; BankPaymentLbl)
                {
                }
                column(CustAndInvoiceNo_Lbl; CustAndInvoiceNoLbl)
                {
                }
                column(Text_ExtendedTextLine; TextExtendedTextLine)
                {
                }
            ...
            begin
                ...
                if cust.GetBillToCustomerNo() <> '' then
                    if Cust."Fin. Charge Terms Code" <> '' then begin
                        FinanceChargeTerms.Get(Cust."Fin. Charge Terms Code");
                        InterestRateFinanceChargeTerms := FinanceChargeTerms."Interest Rate";
                    end;
